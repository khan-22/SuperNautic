...
class Segment {
 public:
  ...
  // Tests a ray collision against all collision surfaces of the segment.
  // Returns collision information temperatures[i] holds temperature of
  // temperature zone i
  const RayIntersection rayIntersectionTest(
 ...
 private:
  ...
  static std::unordered_map<std::string, std::vector<unsigned int> Segment::*>
      nameToVecIndex;
  ...
  // Find smallest and largest vertex position in each axis for a model, using
  // min and max for comparison
  void findMinMaxValues(glm::vec3& min, glm::vec3& max,
                        unsigned int modelIndex);

  // Recursively subdivides an AABB until every box touches <= maxFacesPerBox or
  // maxSubdivisions == 0
  void subdivideOctTree(AABB& box, unsigned int maxFacesPerBox,
  ...
  unsigned int findChildIndex(size_t currentModel, unsigned int index,
                              glm::vec3 boxMiddle);
};
