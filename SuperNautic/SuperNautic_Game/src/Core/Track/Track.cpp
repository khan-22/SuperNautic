...
int Track::getTargetLength() const { return static_cast<int>(_targetLength); }
...
void Track::setSeed(const std::string& seed) {
  ...
  if (seed.length() == 1 && seed[0] == '1') {
    ...
    for (unsigned int i = 0; i < seed.length(); i++) {
      intSeed +=
          (unsigned int)((seed[i] - 65) * static_cast<unsigned int>(
                                              powf(10, static_cast<float>(i))));
    }
    srand(intSeed);
  }
}
...
const std::string& Track::getSeed() const { return _seed; }
...
void Track::startNewTrack() {
  ...
  _octree.reset(new Octree<SegmentInstance*>(
      glm::vec3(0.f, 0.f, 0.f), _targetLength * 1.2f + 10000.f, 2000.f, 5));
...
bool Track::bGenerate() {
  ...
  while (_generatedLength - _lengthAfterLastCall < _progressionLength) {
    ...
    if (index < _segmentHandler->infos().size()) {
      ...
      for (unsigned int i = 0; i < inRow; i++) {
        if (!bInsertNormalSegment(index)) {
          ...
          _endMatrix =
              _track.back()->getModelMatrix() * _track.back()->getEndMatrix();
          break;
        } else {
    ...
    if (newConnection != _endConnection) {
      ...
      _lengthWithCurrentConnectionType +=
          _track.back()->getParent()->getLength();
    }
    ...
    if (_generatedLength > _targetLength - _endMargin) {
      if (bEndTrack()) {
        // End segment
        if (bInsertNormalSegment(
                static_cast<int>(_segmentHandler->infos().size()) - 1)) {
          // Insert end segment
          /*const Segment * segment =
          _segmentHandler->loadSegment(_segmentHandler->infos().size() - 1);
          SegmentInstance* tempInstance = new SegmentInstance(segment,
          _endMatrix); glm::mat4 modelEndMat = segment->getEndMatrix();
...
int Track::getNrOfSegments() const { return static_cast<int>(_track.size()); }

// Get a instance-pointer based on index in the track
SegmentInstance* Track::getInstance(int index) { return _track[index]; }

// Get the track progression in percent [0-1];
float Track::getProgression() const { return _totalProgress; }
...
int Track::getIndex() const {
  ...
  for (unsigned int i = 0; i < infos.size(); i++) {
    if (infos[i]._startConnection == _endConnection &&
      ...
      if (_segmentHandler->getNrOfSegmentsOfType(_endConnection, _curviness) >
          2) {
        if ((infos[i]._endConnection != _endConnection &&
             _lengthWithCurrentConnectionType >= 600.f) ||
  ...
  for (unsigned int i = 0; i < validSegments.size(); i++) {
    if (validSegments[i] < infos.size()) {
      ...
      int prob =
          _segmentHandler->getStructure(index)->getProbability(_curviness);
  ...
  for (unsigned int i = 0; i < validSegments.size(); i++) {
    if (validSegments[i] < infos.size()) {
      ...
      int test = _segmentHandler->getStructure(validSegments[i] - 100)
                     ->getProbability(_curviness);
      if (r - tested < test) {
        return validSegments[i];
      }
      tested += test;
...
glm::vec3 Track::findForward(const glm::vec3 globalPosition,
  ...
  for (long i = static_cast<long>(segmentIndex) - 1;
       i <= static_cast<long>(segmentIndex) + 1; ++i) {
    // Make sure current segment is within bounds
    if (i >= 0 && i < _track.size()) {
      WaypointInfo test = _track[i]->getParent()->findClosestWaypoint(
          glm::inverse(_track[i]->getModelMatrix()) *
          glm::vec4{globalPosition, 1.0f});

      // If test found an appropriate waypoint and closest is empty or test is
      // closer than closest
  ...
  if (!closest.found) {
    LOG_ERROR(
        "No closest waypoint was found! Returning (0, 0, 1) as forward "
        "vector.");
    return glm::vec3{0, 0, 1};
  } else {
    // Return to global world space
    closest.direction = _track[closestIndex]->getModelMatrix() *
                        glm::vec4{closest.direction, 0.0f};
    closest.position = _track[closestIndex]->getModelMatrix() *
                       glm::vec4{closest.position, 1.0f};
  ...
  // Project globalPosition onto the plane defined by closest.direction and
  // closest.position Cast a ray from this position in the direction of
  // closest.direction and find
  //     intersection with plane defined by next.direction and next.distance
  // The length of the ray is used to find interpolation value between waypoint
  // normals
  glm::vec3 projectedGlobalPos{
      globalPosition -
      glm::dot(closest.direction, globalPosition - closest.position) *
          closest.direction};
  ...
  if (divisor <= 0.0f) {
    LOG_ERROR(
        "The direction of two waypoints are orthogonal. Returning direction of "
        "waypoint ahead of position.");
    return next.direction;
  }

  float distanceToPlane =
      (-glm::dot(next.direction, projectedGlobalPos) + planeD) / divisor;

  // [0..1], 0 = at closest waypoint, 1 = at next waypoint
  float interpolationValue =
      glm::dot(closest.direction, globalPosition - closest.position) /
      distanceToPlane;
  if (distanceToPlane <= 0.0f)  // Protect against the case where the track runs
                                // out of waypoints ahead of the player
  {
  ...
  for (unsigned int i = 0; i < closest.index; ++i) {
    lengthInSegment +=
        glm::distance(_track[closestIndex]->getParent()->getWaypoints()[i],
                      _track[closestIndex]->getParent()->getWaypoints()[i + 1]);
  }
  ...
  return glm::normalize(closest.direction * (1 - interpolationValue) +
                        next.direction * interpolationValue);
}
...
bool Track::bInsertNormalSegment(const int index) {
  ...
  int angle = static_cast<int>(
      360.f / _segmentHandler->getConnectionRotation(segment->getStart()));
  int maxRotOffset = segment->getInfo()->getRotationOffset(_curviness) / angle;
  float rotVal = 0.f;
  if (maxRotOffset != 0) {
    rotVal = static_cast<float>((rand() % (2 * maxRotOffset) - maxRotOffset) *
                                angle);
  }
...
bool Track::bInsertIntoOctree(SegmentInstance* segment) {
  ...
  return _octree->bInsertIf(
      mesh, segment, [this](const std::vector<SegmentInstance**>& collisions) {
...
void Track::insertStructure(const int index) {
  ...
  for (int i = 0; i < amount; i++) {
    ...
    for (unsigned int j = 0; j < s->pieces.size(); j++) {
      ...
      float rotVal =
          static_cast<float>(((maxRot - minRot) * scaled + minRot) * angle);
      // Finalizing
      glm::mat4 rotMat =
          glm::rotate(glm::radians(rotVal * rotationDir), glm::vec3(0, 0, 1));
      _endMatrix = _endMatrix * modelEndMat * rotMat;
      _generatedLength += segment->getLength();
      _track.push_back(tempInstance);
...
void Track::addWindowsAndZonesToSegment(const Segment* segment) {
  ...
  if (segment->bHasWindow()) {
    _segmentWindows.push_back({segment->getWindowModel(), _endMatrix,
                               static_cast<unsigned int>(_track.size())});
  }
  ...
  if (segment->nrOfZones() > 0 && _generatedLength > 700.f) {
    ...
    if (nrOfActive >= 1) {
      _temperatureZones.push_back({segment->getZonesModel(), _endMatrix,
                                   static_cast<unsigned int>(_track.size()),
                                   temperatures});
      nrOfActive++;
    }
...
void Track::deleteSegments(const float lengthToDelete) {
  ...
  while (deletedLength <= _deletionLength && _generatedLength > 500.f) {
    ...
    // Windows
    if (_segmentWindows.size() >= 1 &&
        _segmentWindows.back().segmentIndex == index) {
      _segmentWindows.pop_back();
    }
    // Temperature zones
    if (_temperatureZones.size() >= 1 &&
        _temperatureZones.back().segmentIndex == index) {
...
void Track::placeObstacles() {
  ...
  ObstacleHandler::Obstacle* lastObstacleType =
      _obstacleHandler->getRandomObstacle(_difficulty);
  int inRow = 0;
  int currentInRow = 0;
  ...
  while (currentLength < _generatedLength - endLength) {
    if (leftOfArea > 10.f) {
      ...
      float lengthToNextObstacle = rand() % int(30 * invDiff + 10) +
                                   backPadding +
                                   newObstacle->getFrontPadding(_difficulty);
      currentLength += lengthToNextObstacle;
      index = findTrackIndex(currentLength, lastFullSegmentLength);

      std::vector<glm::vec3> waypoints =
          _track[index]->getParent()->getWaypoints();
      waypoints.push_back(
          glm::vec3(_track[index]->getEndMatrix() * glm::vec4(0, 0, 0, 1)));
      ...
      // Position
      glm::vec3 pos =
          waypoints[distanceIndex] + normalize(finalDistance) * remainderDepth;
      // Forward
      ...
      glm::mat4 modelMat =
          _track[index]->getModelMatrix() *
          glm::inverse(glm::lookAt(pos, pos + forward, glm::vec3(0, 1, 0))) *
          glm::rotate(glm::radians(float(rand() % 360)), glm::vec3(0, 0, 1));
      _track[index]->addObstacle(
          ObstacleInstance(modelMat, newObstacle, _difficulty));

      leftOfArea -= lengthToNextObstacle;
    } else {
      currentLength += rand() % (int(500 * invDiff) + 50) + 70;
      leftOfArea =
          static_cast<float>(rand() % (int(600 * _difficulty) + 50) + 100);
    }
  }
}
...
void Track::placeDarkAreas() {
  ...
  while (index < _track.size() - 10) {
    unsigned int length = (unsigned int)(rand() % (int(20 * _difficulty) + 8) +
                                         6 + 10 * _difficulty);
    _darkAreas.push_back({(unsigned int)index, length, 0.f});
    index += length;
    index += (unsigned int)(rand() % (int(300 * (1.f - _difficulty)) + 70) +
                            50 + 100 * (1.f - _difficulty));
  }
  // Removes the last area if it is too long
  if (_darkAreas.size() >= 1 &&
      _darkAreas.back().startIndex + _darkAreas.back().length >
          _track.size() - 10) {
    _darkAreas.erase(_darkAreas.end() - 1);
  }
}

size_t Track::findTrackIndex(const float totalLength,
...
void Track::update(const float dt,
                   const std::vector<unsigned int> playerIndexes) {
  ...
  // Obstacles
  for (unsigned int i = lastPlayer; i < _track.size() && i < firstPlayer + 7;
       ++i) {
    _track[i]->update(dt);
  }
  ...
  for (unsigned int i = 0; i < _temperatureZones.size(); ++i) {
    _track[_temperatureZones[i].segmentIndex]->setTemperatures(
        _temperatureZones[i].temperatures);
  ...
  for (unsigned int i = 0; i < _darkAreas.size(); i++) {
    if (_darkAreas[i].startIndex >= lastPlayer) {
      if (_darkAreas[i].startIndex + _darkAreas[i].length <= firstPlayer) {
        ...
        if (_darkAreas[i].timer > 1.0f &&
            _darkAreas[i].length > (6 + 10 * (1 - _difficulty))) {
  ...
  for (unsigned int i = 0; i < playerIndexes.size(); i++) {
    ...
    if (dapi.startIndex == -1) {
      ...
      for (unsigned int j = 0; j < _darkAreas.size(); j++) {
        if (playerIndexes[i] >= _darkAreas[j].startIndex &&
            playerIndexes[i] <= _darkAreas[j].startIndex + dapi.length) {
...
// Render the track
void Track::render(GFX::ViewportPipeline& pipeline,
...
WaypointInfo Track::findNextWaypointInfo(const WaypointInfo& current,
                                         unsigned int segmentIndex) const {
  WaypointInfo next;

  if (current.index >=
      _track[segmentIndex]->getParent()->getWaypoints().size() - 1) {
    ...
    // Else use first waypoint of next segment
      ...
      next.position =
          _track[segmentIndex + 1]->getModelMatrix() *
          glm::vec4{_track[segmentIndex + 1]->getParent()->getWaypoints()[0],
                    1.0f};

      // Get local position, transform to global space, subtract next.position
      // to get global direction, normalize
      next.direction = glm::normalize(
          glm::vec3{
              (_track[segmentIndex + 1]->getModelMatrix() *
               glm::vec4{
                   _track[segmentIndex + 1]->getParent()->getWaypoints()[1],
                   1.0f})} -
          next.position);
    }
  } else {
    // Use next waypoint in segment
    next.position = _track[segmentIndex]->getModelMatrix() *
                    glm::vec4{_track[segmentIndex]
                                  ->getParent()
                                  ->getWaypoints()[current.index + 1],
                              1.0f};

    if (current.index >=
        _track[segmentIndex]->getParent()->getWaypoints().size() - 2) {
      // Use the end of the segment to find direction of next

      // Get local position, transform to global space, subtract next.position
      // to get global direction, normalize
      next.direction =
          glm::normalize(glm::vec3{(_track[segmentIndex]->getModelMatrix() *
                                    _track[segmentIndex]->getEndMatrix() *
