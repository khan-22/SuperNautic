...
OptionsApplicationState::OptionsApplicationState(ApplicationStateStack& stack,
  ...
  GuiText* resLabel =
      new GuiText("Resolution", FontCache::get("res/arial.ttf"));
  resLabel->setOrigin(resLabel->getBoundingRect().width / 2.f,
                      resLabel->getBoundingRect().height);
  sf::FloatRect resBounds = resolutionList->getBoundingRect();
  resLabel->setPosition(
      resBounds.left + resBounds.width / 2.f,
      resBounds.top - resLabel->getBoundingRect().height / 2.f);

  resolutionList->registerOnSelect(
      [resLabel]() { resLabel->toggleSelection(); });
  resolutionList->registerOnDeselect(
      [resLabel]() { resLabel->toggleSelection(); });
  ...
  GuiButton* fs =
      new GuiButton(sf::Text("Toggle Fullscreen", *_font.get()), [&]() {
  ...
  GuiSlider* musicVolume = new GuiSlider(
      0.f, 1.f, 50.f, 11, sf::Text("Music volume    Low", *_font.get()),
  ...
  GuiSlider* effectsVolume = new GuiSlider(
      0.f, 1.f, 50.f, 11, sf::Text("Effects volume    Low", *_font.get()),
      sf::Text("High", *_font.get()));
  ...
  GuiButton* backButton = new GuiButton(sf::Text("Back", *_font.get()), [&]() {
    _stack.clear();
    _stack.push(std::unique_ptr<ApplicationState>(
        new MainMenuApplicationState(_stack, _context)));
  });
  ...
  _guiContainer.setBackground(sf::Color(27, 173, 222, 100),
                              sf::Color(19, 121, 156, 100), 5.f);
  ...
  _guiContainer.setOrigin(guiBounds.left + guiBounds.width / 2.f,
                          guiBounds.top + guiBounds.height / 2.f);
  _guiContainer.setPosition(windowSize.x / 2.f, windowSize.y / 2.f);

  _toolTip.centerAt(static_cast<size_t>(windowSize.x / 2.f),
                    static_cast<size_t>(windowSize.y * 0.95f));
  _toolTip.registerTip(resolutionList,
                       "Set video resolution. Press A to apply.");
  ...
  _title.setOrigin(_title.getBoundingRect().width / 2.f,
                   _title.getBoundingRect().height / 2.f);
  _title.setPosition(windowSize.x / 2.f,
                     _guiContainer.getBoundingRect().top / 2.f);
...
void OptionsApplicationState::applyOptions() {
  ...
  _toolTip.centerAt(static_cast<size_t>(size.x / 2.f),
                    static_cast<size_t>(size.y * 0.95f));
}
