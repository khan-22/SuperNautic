...
struct BoundingBox {
  ...
  BoundingBox(const glm::vec3& center,
              const std::array<glm::vec3, 3>& directions,
              const std::array<float, 3>& halfLengths)
      : center(center), directions(directions), halfLengths(halfLengths) {}

  // Create a bounding box from a mesh
  BoundingBox(const GFX::RawVertexData& model);

  BoundingBox() : center{glm::vec3{}}, halfLengths{-1.0f, -1.0f, -1.0f} {}
