...
Octree<ElementT>::Octree(const glm::vec3& center, float size, float minNodeSize,
                         unsigned short numElementsPerNode)
    : _bounds(BoundingBox(center,
...
template <typename ElementT>
bool Octree<ElementT>::bInsert(const CollisionMesh& mesh,
                               const ElementT& element) {
  if (_bounds.testCollision(mesh) !=
      CollisionMesh::CollisionResult::COLLISION) {
    return false;
  }
  ...
  typename Node::ElementPtr nodeElement(
      new std::pair<CollisionMesh, ElementT>(mesh, element));
  for (Node* node : nodes) {
...
template <typename ElementT>
bool Octree<ElementT>::bInsertIfNoCollision(const CollisionMesh& mesh,
                                            const ElementT& element) {
  if (_bounds.testCollision(mesh) !=
      CollisionMesh::CollisionResult::COLLISION) {
    return false;
  ...
  for (const typename Node::ElementPtr& nodeElement :
       getUniqueElements(nodes)) {
    if (mesh.testCollision(nodeElement->first) ==
        CollisionMesh::CollisionResult::COLLISION) {
      return false;
    }
  }
...
template <typename ElementT>
bool Octree<ElementT>::bInsertIf(
    const CollisionMesh& mesh, const ElementT& element,
...
template <typename ElementT>
std::vector<ElementT*> Octree<ElementT>::getCollisions(
...
template <typename ElementT>
std::set<typename Octree<ElementT>::Node::ElementPtr>
Octree<ElementT>::getUniqueElements(const std::vector<Node*>& nodes) {
...
Octree<ElementT>::Node::Node(const glm::vec3& center, float size,
                             float minNodeSize, unsigned short numMaxElements)
    : _center(center),
      _size(size),
...
template <typename ElementT>
void Octree<ElementT>::Node::getIntersectingLeafNodes(
    const CollisionMesh& mesh, std::vector<Node*>& leafNodes) const {
  if (bIsLeafNode()) {
  ...
  CollisionMesh::CollisionResult collisionResults[3] = {
      mesh.testCollision(_xPlane), mesh.testCollision(_yPlane),
      mesh.testCollision(_zPlane)};

  static constexpr unsigned char FRONT_MASKS[3] = {0b11110000, 0b11001100,
                                                   0b10101010};
  static_assert(sizeof(FRONT_MASKS[0]) * 8 == _NUM_CHILDREN,
                "Mask bit size must match octree split size.");
...
template <typename ElementT>
const std::vector<typename Octree<ElementT>::Node::ElementPtr>&
Octree<ElementT>::Node::getElements() const {
  return _elements;
}

template <typename ElementT>
bool Octree<ElementT>::Node::bIsCollisionResultValid(
    CollisionMesh::CollisionResult result) const {
  switch (result) {
    case CollisionMesh::CollisionResult::BACK:
    case CollisionMesh::CollisionResult::FRONT:
    case CollisionMesh::CollisionResult::COLLISION:
...
void Octree<ElementT>::Node::split() {
  static const glm::vec3 CHILD_OFFSETS[_NUM_CHILDREN] = {
...
void Octree<ElementT>::Node::erase(const ElementT& element) {
  auto end = std::remove_if(
      ...
      [&element](const ElementPtr& e) { return element == e->second; });
