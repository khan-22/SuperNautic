...
class Octree {
 public:
  ...
  Octree(const glm::vec3& center, float size, float minNodeSize = 0.5f,
         unsigned short numElementsPerNode = 5);

  // Insert element along with a collision mesh.
  // Return true if element is inserted. (insertion fails if mesh does not
  // intersect octree).
  ...
  bool bTestCollision(const CollisionMesh& mesh) const {
    return _bounds.testCollision(mesh) ==
           CollisionMesh::CollisionResult::COLLISION;
  }

  // Hobo erase all elements that compare equal to argument... Does not merge
  // empty nodes. /shrug
 ...
 private:
  class Node {
   public:
    ...
    Node(const glm::vec3& center, float size, float minSize,
         unsigned short numMaxElements);

    void insert(ElementPtr element);
    void erase(const ElementT& element);
    void getIntersectingLeafNodes(const CollisionMesh& mesh,
                                  std::vector<Node*>& leafNodes) const;
  ...
  static std::set<typename Node::ElementPtr> getUniqueElements(
