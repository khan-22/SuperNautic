...
struct AABB {
  ...
  RayIntersection triangleRayIntersection(
      const Ray& ray, const std::vector<GFX::RawVertexData*>& models,
      const std::vector<float>& temperatures) const {
    ...
    for (unsigned i = 0; i < models.size(); ++i) {
      ...
      for (unsigned j = 0; j < currentModel.faces.size(); ++j) {
        ...
        if ((!intersection || t < intersection._length) && t < ray.length() &&
          ...
          if (i < models.size() - 1)  // The last model is the base, others are
                                      // temperature zones
          {
            float temp = temperatures[static_cast<unsigned>(
                currentModel.texCoords[faces[j].x].z)];
          ...
          intersection._normal =
              currentModel.normals[faces[j].x] * (1.0f - u - v) +
  ...
  AABB(glm::vec3 min, glm::vec3 max) : _minCorner{min}, _maxCorner{max} {}
