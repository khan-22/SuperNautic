...
bool bIsSeparatingAxis(const glm::vec3& axis, const BoundingBox& a,
                       const BoundingBox& b);
float computeHalfProjectionLength(const glm::vec3& projectionLine,
                                  const BoundingBox& obb);
...
bool bTestCollision(const BoundingBox& a, const BoundingBox& b) {
  ...
  for (unsigned char i = 0; i < 3; i++) {
    for (unsigned char j = 0; j < 3; j++) {
      if (bIsSeparatingAxis(
              glm::normalize(glm::cross(a.directions[i], b.directions[j])), a,
              b)) {
        return false;
      }
    }
...
bool bTestCollision(const BoundingBox& obb, const Sphere& sphere) {
  ...
  return !(projectionLengthDistance >
           projectionLengthObb + projectionLengthSphere);
}

PlaneCollisionData testCollision(const BoundingBox& obb,
                                 const AxisAlignedPlane& axisAlignedPlane) {
#ifndef NDEBUG
  assertObbCorrectness(obb);
#endif  // NDEBUG
  ...
  float projectionLengthDistance =
      std::fabs(axisAlignedPlane.distance - obb.center[axisIndex]);
  float projectionLengthObb = 0.f;
  for (unsigned char i = 0; i < 3; i++) {
    projectionLengthObb +=
        std::fabs(obb.halfLengths[i] * obb.directions[i][axisIndex]);
  }
...
PlaneCollisionData testCollision(const Sphere& sphere,
                                 const AxisAlignedPlane& axisAlignedPlane) {
#ifndef NDEBUG
  assertSphereCorrectness(sphere);
#endif  // NDEBUG
...
PlaneCollisionData testCollision(const AxisAlignedPlane& axisAlignedPlane1,
                                 const AxisAlignedPlane& axisAlignedPlane2) {
  PlaneCollisionData data;
...
bool bIsSeparatingAxis(const glm::vec3& axis, const BoundingBox& a,
  ...
  if (bIsFloatEq(std::fabs(axis.x) + std::fabs(axis.y) + std::fabs(axis.z),
                 0.f)) {
    return false;
  }
  ...
  float projectionLengthDistance =
      std::fabs(glm::dot(b.center - a.center, axis));
...
float computeHalfProjectionLength(const glm::vec3& projectionLine,
  ...
  float length = 0.f;
  for (unsigned char i = 0; i < 3; i++) {
    length += obb.halfLengths[i] *
              std::fabs(glm::dot(obb.directions[i], projectionLine));
  }
