...
void GuiCharacterInput::renderCurrent(sf::RenderTarget& target,
                                      sf::RenderStates states) const {
  target.draw(_text, states);
}

bool GuiCharacterInput::bIsSelectable() const { return true; }

sf::FloatRect GuiCharacterInput::getBoundingRect() const {
  return getWorldTransform().transformRect(_text.getGlobalBounds());
}

void GuiCharacterInput::selectNextCharacter() { stepCharacter(true); }

void GuiCharacterInput::selectPreviousCharacter() { stepCharacter(false); }
...
void GuiCharacterInput::selectPreviousCharacterList() {
  ...
  if (_currentCharacterListIndex > _characterLists.size()) {
    _currentCharacterListIndex =
        static_cast<unsigned char>(_characterLists.size() - 1);
  }
  updateText();
}
...
GuiCharacterInput::CharacterFlags GuiCharacterInput::getCurrentCharacterList()
    const {
...
void GuiCharacterInput::setCharacter(char character) {
  ...
  if (found != _characterLists.end()) {
    _currentCharacterListIndex = static_cast<unsigned char>(
        std::distance(_characterLists.begin(), found));
...
std::vector<GuiCharacterInput::CharacterFlags>
GuiCharacterInput::generateCharacterLists(CharacterFlags flags) {
