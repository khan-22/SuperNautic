...
struct Camera2D {
  ...
  glm::mat4 perspective =
      glm::infinitePerspective(glm::radians(90.f), 1280.f / 720.f, near);
...
void Line::render(RenderStates& states) {
  ...
  _vao->bind();
  glDrawElements(GL_POINTS, static_cast<GLsizei>(_numPoints), GL_UNSIGNED_INT,
                 0);
  _vao->unbind();
...
void Line::setPoints(const std::vector<glm::vec3>& points) {
  ...
  auto furthestFromCenter = std::max_element(
      points.begin(), points.end(),
      [&center](const glm::vec3& lhs, const glm::vec3& rhs) {
...
void Line::updateModelMatrix() {
  glm::vec3 fitNear =
      camera2D.pos +
      camera2D.dir * (camera2D.near + _radius * _scale.z / _zoom);
  _modelMatrix = glm::translate(fitNear) * _rotation * glm::scale(_scale) *
                 glm::translate(_centerTranslation);
}

void Line::updateScreenPerspectiveMatrix() {
  _screenPerspectiveMatrix =
      glm::translate(glm::vec3(_screenTranslation, 0.f)) * camera2D.perspective;
}

std::shared_ptr<GFX::VertexArrayObject> Line::generateVao(
  ...
  vao->addVertexBuffer(
      static_cast<GLsizei>(pointsSize + colorsSize + sizesSize),
      GL_STATIC_DRAW);

  vao->sendDataToIndexBuffer(0, static_cast<GLsizei>(indicesSize),
                             (void*)indices.data());

  size_t offset = 0;
  vao->sendDataToBuffer(0, 0, static_cast<GLuint>(offset),
                        static_cast<GLsizei>(pointsSize), (void*)points.data(),
                        3, GL_FLOAT);
  offset += pointsSize;

  vao->sendDataToBuffer(0, 1, static_cast<GLuint>(offset),
                        static_cast<GLsizei>(colorsSize), (void*)colors.data(),
                        3, GL_FLOAT);
  offset += colorsSize;

  vao->sendDataToBuffer(0, 2, static_cast<GLuint>(offset),
                        static_cast<GLsizei>(sizesSize), (void*)sizes.data(), 1,
                        GL_FLOAT);
